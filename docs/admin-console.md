...
Set `GYM_ADMIN_STRIPE_SECRET_KEY` in Supabase Edge Function secrets to a live key for that exact account. Existing keys are only reused if the account ID is verified. Required Stripe operations: read account, checkout sessions and line items including payment intent/latest charge; write webhook endpoint only during setup. Do not paste secrets into chat or source control.
...
The production SQL in `supabase/deploy/gym_admin_sync.sql` schedules an import every 15 minutes, catching open and abandoned checkouts even without webhook events. The dashboard shows last successful complete sync and the last error. At initial release the available keys did not validate against this account; Stripe imports/webhook activation require the correct account key. This does not prevent authenticated access to existing gyms, memberships, modules and payment history.
