# Privacy Policy
This Privacy Policy explains how we collect, use, disclose, and protect your personal data when you use the BeFitliner mobile application and related services (the “App”).

By using the App, you agree to the collection and use of information in accordance with this Privacy Policy.

---

## 1. Data Controller

The App is operated by:

**Globalio LLC**  
16192 Coastal Hwy  
Lewes, DE 19958-3608  
United States  
Website: https://www.globaliollc.com  

Globalio LLC is the data controller responsible for your personal data.

---

## 2. Information We Collect

We may collect the following categories of information:

### 2.1 Personal Information
- Name
- Email address
- Account identifiers
- Communication preferences

### 2.2 Fitness & Profile Information
- Gender
- Age
- Height and weight
- Fitness goals and preferences
- Training and activity-related data entered by you

### 2.3 Technical Information
- Device type, operating system, and app version
- IP address
- Log data and usage statistics
- Approximate location (country or region)

### 2.4 Transaction-Related Information
BeFitliner does **not** process payments directly from end users.  
However, we may receive limited transactional metadata (such as confirmation or status) related to:
- gym membership extensions
- training session reservations

All payments are processed by fitness centers, trainers, or third-party platforms.

---

## 3. How We Use Your Information

We use your information to:
- provide and operate the App
- personalize fitness and training recommendations
- facilitate interactions with fitness centers and trainers
- enable gym access and reservations
- communicate important service-related information
- improve performance, usability, and security
- comply with legal obligations

---

## 4. Legal Basis for Processing (GDPR)

Where applicable, we process personal data based on:
- your consent
- performance of a contract
- legitimate business interests
- compliance with legal obligations

---

## 5. Sharing of Information

We may share your data with:
- fitness centers and trainers you interact with
- Apple and Google platforms (app distribution and services)
- email and notification service providers
- analytics and performance monitoring providers
- legal or regulatory authorities when required by law

We do **not** sell your personal data.

---

## 6. Data Retention

We retain personal data only for as long as necessary to:
- provide the App
- fulfill the purposes described in this Policy
- comply with legal, accounting, or regulatory requirements

You may request deletion of your account and personal data at any time.

---

## 7. Data Security

We implement appropriate technical and organizational measures to protect your data against:
- unauthorized access
- loss or misuse
- alteration or disclosure

However, no method of transmission or storage is 100% secure.

---

## 8. Your Rights

Depending on your location, you may have the right to:
- access your personal data
- correct or update your information
- request deletion of your data
- object to or restrict processing
- withdraw consent at any time
- request data portability

Requests can be made by contacting us at the email below.

---

## 9. Children’s Privacy

The App is not intended for children under the age of 18.  
We do not knowingly collect personal data from children.

---

## 10. International Data Transfers

Your information may be processed and stored in countries outside of your residence, including the United States.  
We take appropriate safeguards to ensure data protection.

---

## 11. Changes to This Privacy Policy

We may update this Privacy Policy from time to time.  
Changes are effective when posted, and the “Last updated” date will be revised.

---

## 12. Contact

If you have questions about this Privacy Policy or your personal data, contact us at:

**hello@befitliner.com**
# Privacy Policy

This Privacy Policy explains how we collect, use, disclose, and protect your personal data when you use the BeFitliner mobile application and related services (the “App”).

By using the App, you agree to the collection and use of information in accordance with this Privacy Policy.

---

## 1. Data Controller

The App is operated by:

**Globalio LLC**  
16192 Coastal Hwy  
Lewes, DE 19958-3608  
United States  

Globalio LLC is the data controller responsible for your personal data.

---

## 2. Information We Collect

We collect only the data necessary to operate and improve the App.

### 2.1 Account & Authentication Data
- Email address
- Account identifiers
- Login/session data

### 2.2 Profile & Fitness Data
- Name
- Gender
- Age
- Height and weight
- Fitness goals and preferences
- Training, nutrition, and progress data entered by you

### 2.3 Gym Access & Membership Data
- Gym access activity (e.g., entry logs)
- Membership status and validity
- Interactions with gyms or trainers

### 2.4 Communication & Support Data
- Messages sent within the App (e.g., Messenger)
- Support requests
- Email communication

### 2.5 Technical & Device Data
- Device type and operating system
- App version
- IP address
- Log data
- Approximate location (country/region)

### 2.6 Analytics & Performance Data
- App usage statistics
- Crash reports
- Performance diagnostics

---

## 3. How We Use Your Information

We use your data to:
- provide and operate the App
- enable gym access and memberships
- personalize fitness recommendations and progress tracking
- enable communication with gyms and trainers
- provide customer support
- improve app performance and user experience
- ensure security and prevent abuse
- comply with legal obligations

---

## 4. Legal Basis for Processing (GDPR)

Where applicable, we process personal data based on:
- your consent
- performance of a contract
- legitimate business interests
- compliance with legal obligations

---

## 5. Sharing of Information

We may share your data with:

- fitness centers and trainers you interact with
- infrastructure and hosting providers (e.g., backend services)
- analytics and performance monitoring providers
- communication and notification providers
- legal or regulatory authorities when required by law

We do **not** sell your personal data.

---

## 6. Third-Party Services

We may use third-party services to operate and improve the App, including:
- backend infrastructure (e.g., database and authentication)
- push notifications
- analytics and crash reporting

These providers process data only as necessary to provide their services to us.

---

## 7. Data Retention

We retain data only as long as necessary:

- Account data: while your account is active
- Fitness and profile data: until deleted by you or upon account deletion
- Communication data: as needed for support and legal purposes
- Technical and analytics data: for a limited period to improve the App

---

## 8. Data Deletion

You have the right to request deletion of your personal data at any time.

You can request deletion by:
- contacting us at **hello@befitliner.com**

Upon request, we will delete your account and associated data, unless retention is required for legal or security purposes.

---

## 9. Data Security

We implement appropriate technical and organizational measures to protect your data against:
- unauthorized access
- loss or misuse
- alteration or disclosure

However, no system is completely secure.

---

## 10. Your Rights

Depending on your location, you may have the right to:
- access your personal data
- correct inaccurate data
- request deletion
- restrict or object to processing
- withdraw consent
- request data portability

You can exercise your rights by contacting us.

---

## 11. Children’s Privacy

The App is not intended for users under 18 years of age.
We do not knowingly collect data from children.

---

## 12. International Data Transfers

Your data may be processed in countries outside your residence, including the United States.
We apply appropriate safeguards to protect your data.

---

## 13. Changes to This Privacy Policy

We may update this Privacy Policy from time to time.
Changes are effective when published.

---

## 14. Fitliner Health, payments and marketing measurement

When you use the Health Card, we process health results, biomarkers, reference ranges, documents and onboarding questionnaire answers that you provide. These are sensitive health data. We process them with your explicit consent and to provide the features you request. You can review extracted values before they are saved.

The Fitliner Health digital service is provided by Globalio LLC. For subscriptions purchased through Managed Payments, Link acts as merchant of record for the transaction and Stripe/Link handles payment, billing documents and transaction support. We do not store your full payment card number. We may retain subscription status, billing identifiers and transaction confirmation to manage access, provide support, keep accounting records and comply with legal obligations.

To evaluate advertising, we may store non-identifying campaign names from the link (such as UTM parameters) in our own system. On Health Card pages, we do not send Meta your health results, questionnaire answers, email address or purchase information through Meta Pixel or Conversions API.

## 15. Contact

If you have questions or requests regarding your personal data, contact us at:

**hello@befitliner.com**
